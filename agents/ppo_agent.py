...
from agents.agents import state_to_tensor
...
class PPO_Agent(Agent):
    ...
    def train(self, experience):
        ...
        L_clip_loss = -torch.min(
            rt * advantage,
            torch.clamp(rt, 1 - self.epsilon, 1 + self.epsilon) * advantage,
        )
        ...
        value_loss = self.value_loss(
            self.predict(obs)[1], torch.flatten(torch.Tensor(rewards))
        )
